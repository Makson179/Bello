...
Return exactly one concise, human-readable setup in the user's language, following [OUTPUT_CONTRACT.md](references/OUTPUT_CONTRACT.md). Include the exact task path, planning decision, active role profiles, the four independent switches, review schedule, sub-agent policies, speed, triage, and workspace handling.
...
- Apply changed runtime/distiller switches to a fresh run; do not resume existing threads under a changed tool contract. Report the fresh-run requirement rather than silently clearing history or restarting.

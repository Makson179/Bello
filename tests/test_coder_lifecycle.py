...
from supervisor.coder import CODER_INTERRUPT_RPC_TIMEOUT_SECONDS, CoderSession
...
async def test_coder_interrupt_uses_cleanup_deadline_not_coder_deadline(tmp_path: Path) -> None:
    ...
    assert calls == [("thread", "turn", CODER_INTERRUPT_RPC_TIMEOUT_SECONDS)]
    assert CODER_INTERRUPT_RPC_TIMEOUT_SECONDS == 600.0
    # The coder override does not silently change other app-server cleanup RPCs.
